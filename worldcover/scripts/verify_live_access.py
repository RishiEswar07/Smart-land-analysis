...
--------------------------------
Run this ON YOUR OWN MACHINE (not in a network-restricted sandbox) to
confirm you can actually reach the real ESA WorldCover S3 bucket and
get a real classification back. This is the live counterpart to
tests/test_landcover_service.py, which only proves the code logic is
correct using a synthetic file (it can't reach AWS from this sandbox).
...
# A handful of well-known Indian locations with an obvious expected
# land-cover category, so a wrong answer is easy to spot.
CHECKPOINTS = [
