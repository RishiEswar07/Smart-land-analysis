...
----------------------------
Since this sandbox cannot reach AWS S3 (network egress restricted),
this test does NOT hit the real ESA WorldCover data. Instead, it builds
a small SYNTHETIC GeoTIFF — matching the exact tile-naming convention
and CRS the real service expects — with KNOWN pixel values placed at
KNOWN coordinates, then runs the real, unmodified `get_land_cover()`
production function against it in `local_dir` mode.

This proves the sampling/indexing/classification code is correct.
It does NOT prove the real S3 URLs are reachable from your machine —
you should run scripts/verify_live_access.py (also provided) once on
your own machine/server to confirm that separately.
"""
...
def build_synthetic_tile(tmp_dir: str, lat: float, lon: float, class_code: int) -> str:
    ...
    # Real tiles are ~10m resolution (0.0000833... deg/px); using a much
    # coarser grid here is fine — we only need correct georeferencing,
    # not real resolution, to prove the indexing logic.
    width = height = 300
...
def run():
    tmp_dir = "/tmp/synthetic_worldcover"
    os.makedirs(tmp_dir, exist_ok=True)

    test_cases = [
        # (lat, lon, class_code, expected_name, expected_category, expected_suitability)
        (9.9252, 78.1198, 50, "Built-up", "Built-up", "Suitable"),           # Madurai coords, forced Built-up
        (11.0168, 76.9558, 10, "Tree cover", "Tree cover", "Caution"),       # Coimbatore coords, forced Tree cover
        (13.0827, 80.2707, 80, "Permanent water bodies", "Water", "Unsuitable"),  # Chennai coords, forced Water
        (28.6139, 77.2090, 90, "Herbaceous wetland", "Wetland", "Unsuitable"),    # Delhi coords, forced Wetland
        (26.9124, 75.7873, 40, "Cropland", "Cropland", "Caution"),           # Jaipur coords, forced Cropland
    ]
    ...
    # No-data (code 0) should raise a clear error, not silently succeed
